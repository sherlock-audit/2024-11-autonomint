    ...
    OwnableUpgradeable
{
    ...
    uint128 public lastEthPrice; // Last updated ETH price
    ...
    bool public downsideProtectedSign;
    ...
    function setAdmin(address adminAddress) external onlyOwner {
        ...
        if (adminAddress == address(0) || isContract(adminAddress))
            revert CDS_CantBeContractOrZeroAddress(adminAddress);
        // Check whether, the function have required approvals from owners to set
        if (!multiSign.executeSetterFunction(IMultiSign.SetterFunctions(4)))
            revert CDS_RequiredApprovalsNotMetToSet();
        admin = adminAddress;
    }
    ...
    function setAdminTwo(bytes32 hashedAddress) external onlyAdmin {
        // Check whether the input address is not a zero address and contract
        if (hashedAddress == keccak256(abi.encodePacked(address(0))))
            revert CDS_CantBeZeroAddress(hashedAddress);
    ...
    function deposit(
        ...
        totalCdsDepositedAmountWithOptionFees = result
            .totalCdsDepositedAmountWithOptionFees;
        totalAvailableLiquidationAmount = result
            .totalAvailableLiquidationAmount;
    ...
    function withdraw(
        ...
        CdsAccountDetails memory cdsDepositDetails = cdsDetails[msg.sender]
        ...
        (uint128 weETH_ExchangeRate /* uint128 ethPrice*/, ) = borrowing
            .getUSDValue(borrowing.assetAddress(IBorrowing.AssetName.WeETH));
        ...
        uint256 optionFees = ((cdsDepositDetails.normalizedAmount *
            omniChainData.lastCumulativeRate) / (CDSLib.PRECISION * CDSLib.NORM_PRECISION)) -
        ...
        if (currentValue > cdsDepositDetails.depositedAmount) {
            currentValue = cdsDepositDetails.depositedAmount;
        }
        ...
        totalCdsDepositedAmountWithOptionFees = withdrawResult
            .totalCdsDepositedAmountWithOptionFees;
        withdrawResult.omniChainData.cdsPoolValue -= cdsDepositDetails
            .depositedAmount; //!
        // Update the user deposit data
        cdsDetails[msg.sender].cdsAccountDetails[index] = withdrawResult
            .cdsDepositDetails;
        ...
        if (
            withdrawResult.omniChainData.totalVolumeOfBorrowersAmountinWei != 0
        ) {
            if (
                borrowing.calculateRatio(0, ethPrice) <
                (2 * CDSLib.RATIO_PRECISION)
            ) revert CDS_NotEnoughFundInCDS();
        }
        ...
        if (
            optionsFeesToGetFromOtherChain == 0 && withdrawResult.ethAmount == 0
        ) {
    ...
    function cdsAmountToReturn(
        ...
        uint256 depositedAmount = cdsDetails[_user]
            .cdsAccountDetails[index]
            .depositedAmount;
        uint128 cumulativeValueAtDeposit = cdsDetails[msg.sender]
            .cdsAccountDetails[index]
            .depositValue;
        // Get the cumulative value sign at the time of deposit
        bool cumulativeValueSignAtDeposit = cdsDetails[msg.sender]
            .cdsAccountDetails[index]
            .depositValueSign;
        uint128 valDiff;
    ...
    function setWithdrawTimeLimit(uint64 _timeLimit) external onlyAdmin {
        ...
        // Check whether, the function have required approvals from owners to set
        if (!multiSign.executeSetterFunction(IMultiSign.SetterFunctions(2)))
            revert CDS_RequiredApprovalsNotMetToSet();
    ...
    function setBorrowingContract(address _address) external onlyAdmin {
        ...
        if (_address == address(0) || !isContract(_address))
            revert CDS_CantBeEOAOrZeroAddress(_address);
    ...
    function setTreasury(address _treasury) external onlyAdmin {
        ...
        if (_treasury == address(0) || !isContract(_treasury))
            revert CDS_CantBeEOAOrZeroAddress(_treasury);
        // Check whether, the function have required approvals from owners to set
        if (!multiSign.executeSetterFunction(IMultiSign.SetterFunctions(6)))
            revert CDS_RequiredApprovalsNotMetToSet();
    ...
    function setUSDaLimit(uint8 percent) external onlyAdmin {
        ...
        // Check whether, the function have required approvals from owners to set
        if (!multiSign.executeSetterFunction(IMultiSign.SetterFunctions(8)))
            revert CDS_RequiredApprovalsNotMetToSet();
        usdaLimit = percent;
    }
    ...
    function setUsdtLimit(uint64 amount) external onlyAdmin {
        ...
        // Check whether, the function have required approvals from owners to set
        if (!multiSign.executeSetterFunction(IMultiSign.SetterFunctions(9)))
            revert CDS_RequiredApprovalsNotMetToSet();
        usdtLimit = amount;
    }
    ...
    function calculateCumulativeValue(
        ...
        returns (CalculateValueResult memory result)
    {
        return
            _calculateCumulativeValue(
                vaultBal,
    ...
    function getOptionsFeesProportions(
        ...
        return
            CDSLib.getOptionsFeesProportions(
                optionsFees,
                totalCdsDepositedAmount,
                omniChainData.totalCdsDepositedAmount -
                    omniChainData.downsideProtected,
    ...
    function calculateLiquidatedETHTogiveToUser(
        ...
        CDSInterface.CdsAccountDetails memory cdsDepositData = cdsDetails[user]
            .cdsAccountDetails[index];
        IGlobalVariables.OmniChainData memory omniChainData = globalVariables
            .getOmniChainData();
        uint256 ethAmount; //! local variable never initialized, but it is initialized in below if
        uint128 profit; //! local variable never initialized, but it is initialized in below if
        ...
        uint256 returnAmount = (cdsDepositData.normalizedAmount *
            omniChainData.lastCumulativeRate) / CDSLib.PRECISION;
        if (cdsDepositData.optedLiquidation) {
            ...
            if (currentLiquidations >= liquidationIndexAtDeposit) {
                ...
                for (
                    ...
                    i <= currentLiquidations;
                    i++
                ) {
                    uint128 liquidationAmount = cdsDepositData
                        .liquidationAmount;
                    if (liquidationAmount > 0) {
                        ...
                        profit += (liquidationData.profits * share) / 1e10; //! performs a multiplication on the result of a division
                        cdsDepositData.liquidationAmount -= ((liquidationData
                            .liquidationAmount * share) / 1e10); //! performs a multiplication on the result of a division
                        ethAmount +=
                            (liquidationData.collateralAmount * share) /
                            1e10; //! performs a multiplication on the result of a division
                    }
                }
            }
            returnAmount += cdsDepositData.liquidationAmount;
    ...
    function getTotalCdsDepositedAmountWithOptionFees()
        public
        view
        returns (uint256)
    {
    ...
    function verify(        
        ...
        return _verify(FunctionName.BORROW_WITHDRAW, 0, 0, odosExecutionData, signature);
    }
    ...
    function _verify(
        ...
        if (functionName == FunctionName.CDS_WITHDRAW) {
            digest = _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        ...
                        excessProfitCumulativeValue,nonce
                    )
                )
            );
            ...
            digest = _hashTypedDataV4(
                keccak256(
                    abi.encode(keccak256("OdosPermit(bytes odosExecutionData)"), odosExecutionData)
                )
            );
        }
