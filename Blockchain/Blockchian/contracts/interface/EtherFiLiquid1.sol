...
interface EtherFiLiquid1 {
    ...
    error Cellar__TotalAssetDeviatedOutsideRange(
        uint256 assets,
        uint256 min,
        uint256 max
    );
    error Cellar__TotalSharesMustRemainConstant(
        uint256 current,
        uint256 expected
    );
    ...
    event StrategistPayoutAddressChanged(
        address oldPayoutAddress,
        address newPayoutAddress
    );
    event StrategistPlatformCutChanged(
        uint64 oldPlatformCut,
        uint64 newPlatformCut
    );
