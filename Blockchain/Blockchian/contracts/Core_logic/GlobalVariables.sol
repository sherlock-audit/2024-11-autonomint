    ...
    OAppUpgradeable
{
    ...
    mapping(IBorrowing.AssetName assetName => CollateralData collateralData)
        private s_collateralData; // Individual collateral data
    ...
    function setTreasury(address _treasury) public onlyOwner {
        ...
        require(
            _treasury != address(0) && isContract(_treasury),
    ...
    function setBorrowing(address _borrow) public onlyOwner {
        // Check whether the input address is not a zero address and EOA
        require(
            _borrow != address(0) && isContract(_borrow),
    ...
    function setBorrowLiq(address _borrowLiq) public onlyOwner {
        // Check whether the input address is not a zero address and EOA
        require(
            _borrowLiq != address(0) && isContract(_borrowLiq),
        ...
        returns (MessagingReceipt memory receipt)
    {
        ...
        bytes memory _payload = abi.encode(
            functionToDo,
            callingFunction,
            callingFunction == CallingFunction.CDS_WITHDRAW
                ? oftTransferData.tokensToSend
                : 0,
            callingFunction == CallingFunction.BORROW_WITHDRAW
                ? oftTransferData.tokensToSend
                : 0,
            callingFunction == CallingFunction.BORROW_LIQ
                ? oftTransferData.tokensToSend
                : 0,
        ...
        options = OptionsBuilder
            .newOptions()
            .addExecutorLzReceiveOption(280000, 0)
        ...
        if (functionToDo == FunctionToDo.TOKEN_TRANSFER) {
            ...
            _options = OptionsBuilder
                ...
                .addExecutorNativeDropOption(
                    ...
                    uint128(
                        2 *
                            feeForTokenTransfer.nativeFee +
                            (feeForCollateralTransfer.nativeFee -
            ...
            _options = OptionsBuilder
                .newOptions()
                .addExecutorLzReceiveOption(700000, 0)
                .addExecutorNativeDropOption(
    ...
    function _lzReceive(
        ...
        if (oappData.functionToDo == FunctionToDo.TOKEN_TRANSFER) {
            ...
            treasury.transferFundsToGlobal(
                [0, 0, 0, oappData.oftTransferData.tokensToSend]
            );
            ...
            oappData.functionToDo == FunctionToDo.COLLATERAL_TRANSFER ||
            oappData.functionToDo == FunctionToDo.BOTH_TRANSFER
        ) {
            ...
            _options = OptionsBuilder
                ...
                .addExecutorNativeDropOption(
                    ...
                    bytes32(
                        uint256(
                            uint160(
                                oappData.collateralTokenTransferData.recipient
                            )
                        )
                    )
                );
            ...
            for (uint8 i = 0; i < tokensToSend.length; i++) {
                if (tokensToSend[i] > 0) {
                    ...
                    if (i == 1) {
                        assetAddress = borrowInstance.assetAddress(
        ...
        cds.updateTotalCdsDepositedAmount(
            uint128(oappData.cdsAmountToRemove + oappData.liqAmountToRemove)
        );
