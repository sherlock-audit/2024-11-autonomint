...
library CDSLib {
    ...
    function calculateCumulativeValue(
        ...
        if (totalCdsDepositedAmount == 0) {
            ...
            value = uint128(
                (_amount * vaultBal * priceDiff * 1e6) /
                    (PRECISION * totalCdsDepositedAmount)
            );
        }
    ...
    function getOptionsFeesProportions(
        ...
        uint256 totalOptionFeesInThisChain = _totalCdsDepositedAmountWithOptionFees -
                _totalCdsDepositedAmount;
        // calculate share of both chains
        uint256 share = (otherChainCDSAmount * 1e10) /
        ...
        if (totalOptionFeesInOtherChain == 0) {
            ...
            if (totalOptionFeesInOtherChain < optionsfeesToGet) {
                ...
                if (
                    ...
                    optionsfeesToGet +=
                        optionsFeesRemaining -
                        totalOptionFeesInThisChain;
                } else {
    ...
    function getLiquidatedCollateralToGive(
        ...
        uint256 totalAmountNeededInETH = param.ethAmountNeeded +
            (param.weETHAmountNeeded * param.weETHExRate) /
            1 ether +
            (param.rsETHAmountNeeded * param.rsETHExRate) /
            1 ether;
        // calculate amount needed in weeth value
        uint256 totalAmountNeededInWeETH = (totalAmountNeededInETH * 1 ether) /
            param.weETHExRate;
        // calculate amount needed in rseth value
        uint256 totalAmountNeededInRsETH = (totalAmountNeededInETH * 1 ether) /
            param.rsETHExRate;
        ...
        if (
            param.totalCollateralAvailableInETHValue >= totalAmountNeededInETH
        ) {
            ...
            if (param.ethAvailable >= totalAmountNeededInETH) {
                ...
                liquidatedCollateralToGiveInWeETH =
                    ((totalAmountNeededInETH -
                        liquidatedCollateralToGiveInETH) * 1 ether) /
                    param.weETHExRate;
                if (param.weETHAvailable < liquidatedCollateralToGiveInWeETH) {
                    ...
                    liquidatedCollateralToGiveInRsETH =
                        ((totalAmountNeededInETH -
                            liquidatedCollateralToGiveInETH -
                            (liquidatedCollateralToGiveInWeETH *
                                param.weETHExRate) /
                            1 ether) * 1 ether) /
                        param.rsETHExRate;
            ...
            liquidatedCollateralToGetFromOtherChainInETHValue =
                totalAmountNeededInETH -
                param.totalCollateralAvailableInETHValue;
        }
        return (
    ...
    function deposit(
        ...
        if (totalDepositingAmount == 0)
            revert CDSInterface.CDS_ShouldNotBeZero(); // check _amount not zero
        // Check the liquidationAmount is lesser than totalDepositingAmount
        if (params.liquidationAmount > totalDepositingAmount)
            revert CDSInterface.CDS_LiqAmountExceedsDepositAmount(
        ...
        if (omniChainData.usdtAmountDepositedTillNow < params.usdtLimit) {
            ...
            if (
                ...
                if (params.usdtAmount != totalDepositingAmount)
                    revert CDSInterface.CDS_NeedsUSDTOnly();
            } else {
            ...
            if (interfaces.usda.balanceOf(msg.sender) < params.usdaAmount)
                revert CDSInterface.CDS_Insufficient_USDa_Balance(); // check if user has sufficient USDa token
        }
        // Check the eth price is non zero
        if (params.ethPrice == 0)
            revert CDSInterface.CDS_ETH_PriceFeed_Failed();
        uint64 index;
        ...
        CDSInterface.CdsAccountDetails memory cdsDepositDetails = cdsDetails[
            msg.sender
        ].cdsAccountDetails[index];
        ...
        if (omniChainData.downsideProtected > 0) {
            omniChainData.totalCdsDepositedAmount -= omniChainData
                .downsideProtected;
            omniChainData.totalCdsDepositedAmountWithOptionFees -= omniChainData
                .downsideProtected;
        ...
        cdsDepositDetails.normalizedAmount = ((totalDepositingAmount *
            PRECISION *
            NORM_PRECISION) / (omniChainData.lastCumulativeRate));
        ...
        if (params.liquidate) {
            ...
            cdsDepositDetails.initialLiquidationAmount = params
                .liquidationAmount;
            params.totalAvailableLiquidationAmount += params.liquidationAmount;

            // updating global data
            omniChainData.totalAvailableLiquidationAmount += params
                .liquidationAmount;
        }
        ...
        if (params.usdtAmount != 0 && params.usdaAmount != 0) {
            if (interfaces.usdt.balanceOf(msg.sender) < params.usdtAmount)
                revert CDSInterface.CDS_Insufficient_USDT_Balance(); // check if user has sufficient USDa token
    ...
    function withdrawUser(
        ...
        if (!params.cdsDepositDetails.optedLiquidation) {
            ...
            uint128 liquidationIndexAtDeposit = params
                .cdsDepositDetails
                .liquidationindex;
            ...
            if (
                ...
                for (
                    uint128 i = (liquidationIndexAtDeposit + 1);
                    i <= params.omniChainData.noOfLiquidations;
                    i++
                ) {
                    uint128 liquidationAmount = params
                        .cdsDepositDetails
                        .liquidationAmount;
                    // If the user available liquidation is non zero
                    if (liquidationAmount > 0) {
                        CDSInterface.LiquidationInfo
                            memory liquidationData = omniChainCDSLiqIndexToInfo[
                                i
                            ];
                        // Calculate the share by taking ratio between
                        // User's available liquidation amount and total available liquidation amount
                        uint128 share = (liquidationAmount * 1e10) /
                            uint128(liquidationData.availableLiquidationAmount);
                        // Update users available liquidation amount
                        params
                            .cdsDepositDetails
                            .liquidationAmount -= getUserShare(
                            liquidationData.liquidationAmount,
                            share
                        );
                        // Based on the collateral type calculate the liquidated collateral to give to user
                        if (
                            liquidationData.assetName ==
                            IBorrowing.AssetName.ETH
                        ) {
                            // increment eth amount
                            params.ethAmount += getUserShare(
                            ...
                            weETHAmount += getUserShare(
                                liquidationData.collateralAmount,
                                share
                            );
                            weETHAmountInETHValue += getUserShare(
                                liquidationData.collateralAmountInETHValue,
                                share
                            );
                        } else if (
                            liquidationData.assetName ==
                            ...
                            rsETHAmount += getUserShare(
                                liquidationData.collateralAmount,
                                share
                            );
                            rsETHAmountInETHValue += getUserShare(
                                liquidationData.collateralAmountInETHValue,
                                share
                            );
                        }
                ...
                uint256 returnAmountWithGains = params.returnAmount +
                    params.cdsDepositDetails.liquidationAmount;
                returnAmountWithGains -= params
                    .cdsDepositDetails
                ...
                interfaces.treasury.updateYieldsFromLiquidatedLrts(
                    weETHAmount -
                        ((weETHAmountInETHValue * 1 ether) /
                            params.weETH_ExchangeRate) +
                        rsETHAmount -
                        ((rsETHAmountInETHValue * 1 ether) /
                ...
                (
                    ...
                    CDSInterface.GetLiquidatedCollateralToGiveParam(
                        ...
                        weETHAmount,
                        rsETHAmount,
                        interfaces.treasury.liquidatedCollateralAmountInWei(
                            IBorrowing.AssetName.ETH
                        ),
                        interfaces.treasury.liquidatedCollateralAmountInWei(
                            IBorrowing.AssetName.WeETH
                ...
                if (
                    ...
                    if (
                        ...
                        totalCdsDepositedAmount -= params
                            .cdsDepositDetails
                            .depositedAmount;
                    } else {
                    ...
                    params.omniChainData.totalCdsDepositedAmount -= params
                        .cdsDepositDetails
                        .depositedAmount;
                    ...
                    params
                        ...
                        .totalCdsDepositedAmountWithOptionFees -= (params
                        .cdsDepositDetails
                        .depositedAmount + params.optionFees);
                } else {
                    // update totalCdsDepositedAmount
                    totalCdsDepositedAmount -= (params
                        .cdsDepositDetails
                ...
                if (
                    ...
                    if (collateralToGetFromOtherChain != 0) {
                        ...
                        (
                            ...
                            CDSInterface.GetLiquidatedCollateralToGiveParam(
                                ...
                                interfaces
                                    .globalVariables
                                    .getOmniChainCollateralData(
                                        IBorrowing.AssetName.ETH
                                    )
                                    .totalLiquidatedAmount -
                                    interfaces
                                        .treasury
                                        .liquidatedCollateralAmountInWei(
                                            IBorrowing.AssetName.ETH
                                        ),
                                interfaces
                                    .globalVariables
                    ...
                    if (rsETHAmountFromOtherChain > 0) {
                        interfaces.treasury.wrapRsETH(
                            rsETHAmountFromOtherChain
                        );
                    }
                }
                //Calculate the usda amount to give to user after deducting 10% from the above final amount
                params.usdaToTransfer = calculateUserProportionInWithdraw(
                    params.cdsDepositDetails.depositedAmount,
                    returnAmountWithGains
                );
                //Update the treasury data
                interfaces.treasury.updateUsdaCollectedFromCdsWithdraw(
                    returnAmountWithGains - params.usdaToTransfer
                );
                interfaces.treasury.updateLiquidatedETHCollectedFromCdsWithdraw(
                        params.ethAmount
                    );
                // Update deposit data
                params.cdsDepositDetails.withdrawedAmount = params
                    .usdaToTransfer;
                params
                    .cdsDepositDetails
                    .withdrawCollateralAmount = totalWithdrawCollateralAmountInETH;
                params.cdsDepositDetails.optionFees = params.optionFees;
                params.cdsDepositDetails.optionFeesWithdrawn = params
                    .optionFees;
                ...
                if (!success)
                    revert CDSInterface.CDS_TransferFailed(
                        IBorrowing.AssetName.USDa
                    );
                ...
                if (params.ethAmount != 0) {
                    params
                        .omniChainData
                        .collateralProfitsOfLiquidators -= totalWithdrawCollateralAmountInETH;
                ...
                if (weETHAmount != 0) {
                    ...
                    bool sent = IERC20(
                        interfaces.borrowing.assetAddress(
                            IBorrowing.AssetName.WeETH
                        )
                    ).transferFrom(
                            address(interfaces.treasury),
    ...
    function withdrawUserWhoNotOptedForLiq(
        ...
        params.omniChainData.totalCdsDepositedAmount -= params
            .cdsDepositDetails
            .depositedAmount;
        params.omniChainData.totalCdsDepositedAmountWithOptionFees -= params
            .returnAmount;
